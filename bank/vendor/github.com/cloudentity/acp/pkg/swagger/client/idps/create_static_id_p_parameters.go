// Code generated by go-swagger; DO NOT EDIT.

package idps

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"
	"net/http"
	"time"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/runtime"
	cr "github.com/go-openapi/runtime/client"
	"github.com/go-openapi/strfmt"

	"github.com/cloudentity/acp/pkg/swagger/models"
)

// NewCreateStaticIDPParams creates a new CreateStaticIDPParams object
// with the default values initialized.
func NewCreateStaticIDPParams() *CreateStaticIDPParams {
	var (
		aidDefault = string("default")
		tidDefault = string("default")
	)
	return &CreateStaticIDPParams{
		Aid: aidDefault,
		Tid: tidDefault,

		timeout: cr.DefaultTimeout,
	}
}

// NewCreateStaticIDPParamsWithTimeout creates a new CreateStaticIDPParams object
// with the default values initialized, and the ability to set a timeout on a request
func NewCreateStaticIDPParamsWithTimeout(timeout time.Duration) *CreateStaticIDPParams {
	var (
		aidDefault = string("default")
		tidDefault = string("default")
	)
	return &CreateStaticIDPParams{
		Aid: aidDefault,
		Tid: tidDefault,

		timeout: timeout,
	}
}

// NewCreateStaticIDPParamsWithContext creates a new CreateStaticIDPParams object
// with the default values initialized, and the ability to set a context for a request
func NewCreateStaticIDPParamsWithContext(ctx context.Context) *CreateStaticIDPParams {
	var (
		aidDefault = string("default")
		tidDefault = string("default")
	)
	return &CreateStaticIDPParams{
		Aid: aidDefault,
		Tid: tidDefault,

		Context: ctx,
	}
}

// NewCreateStaticIDPParamsWithHTTPClient creates a new CreateStaticIDPParams object
// with the default values initialized, and the ability to set a custom HTTPClient for a request
func NewCreateStaticIDPParamsWithHTTPClient(client *http.Client) *CreateStaticIDPParams {
	var (
		aidDefault = string("default")
		tidDefault = string("default")
	)
	return &CreateStaticIDPParams{
		Aid:        aidDefault,
		Tid:        tidDefault,
		HTTPClient: client,
	}
}

/*CreateStaticIDPParams contains all the parameters to send to the API endpoint
for the create static ID p operation typically these are written to a http.Request
*/
type CreateStaticIDPParams struct {

	/*StaticIDP
	  StaticIDP

	*/
	StaticIDP *models.StaticIDP
	/*Aid
	  Authorization server id

	*/
	Aid string
	/*Tid
	  Tenant id

	*/
	Tid string

	timeout    time.Duration
	Context    context.Context
	HTTPClient *http.Client
}

// WithTimeout adds the timeout to the create static ID p params
func (o *CreateStaticIDPParams) WithTimeout(timeout time.Duration) *CreateStaticIDPParams {
	o.SetTimeout(timeout)
	return o
}

// SetTimeout adds the timeout to the create static ID p params
func (o *CreateStaticIDPParams) SetTimeout(timeout time.Duration) {
	o.timeout = timeout
}

// WithContext adds the context to the create static ID p params
func (o *CreateStaticIDPParams) WithContext(ctx context.Context) *CreateStaticIDPParams {
	o.SetContext(ctx)
	return o
}

// SetContext adds the context to the create static ID p params
func (o *CreateStaticIDPParams) SetContext(ctx context.Context) {
	o.Context = ctx
}

// WithHTTPClient adds the HTTPClient to the create static ID p params
func (o *CreateStaticIDPParams) WithHTTPClient(client *http.Client) *CreateStaticIDPParams {
	o.SetHTTPClient(client)
	return o
}

// SetHTTPClient adds the HTTPClient to the create static ID p params
func (o *CreateStaticIDPParams) SetHTTPClient(client *http.Client) {
	o.HTTPClient = client
}

// WithStaticIDP adds the staticIDP to the create static ID p params
func (o *CreateStaticIDPParams) WithStaticIDP(staticIDP *models.StaticIDP) *CreateStaticIDPParams {
	o.SetStaticIDP(staticIDP)
	return o
}

// SetStaticIDP adds the staticIdP to the create static ID p params
func (o *CreateStaticIDPParams) SetStaticIDP(staticIDP *models.StaticIDP) {
	o.StaticIDP = staticIDP
}

// WithAid adds the aid to the create static ID p params
func (o *CreateStaticIDPParams) WithAid(aid string) *CreateStaticIDPParams {
	o.SetAid(aid)
	return o
}

// SetAid adds the aid to the create static ID p params
func (o *CreateStaticIDPParams) SetAid(aid string) {
	o.Aid = aid
}

// WithTid adds the tid to the create static ID p params
func (o *CreateStaticIDPParams) WithTid(tid string) *CreateStaticIDPParams {
	o.SetTid(tid)
	return o
}

// SetTid adds the tid to the create static ID p params
func (o *CreateStaticIDPParams) SetTid(tid string) {
	o.Tid = tid
}

// WriteToRequest writes these params to a swagger request
func (o *CreateStaticIDPParams) WriteToRequest(r runtime.ClientRequest, reg strfmt.Registry) error {

	if err := r.SetTimeout(o.timeout); err != nil {
		return err
	}
	var res []error

	if o.StaticIDP != nil {
		if err := r.SetBodyParam(o.StaticIDP); err != nil {
			return err
		}
	}

	// path param aid
	if err := r.SetPathParam("aid", o.Aid); err != nil {
		return err
	}

	// path param tid
	if err := r.SetPathParam("tid", o.Tid); err != nil {
		return err
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}
